...
checkpoints = [50, 100, 200, 300, 400, 500, 1000, 5000, 10000]
...
def generate_spiral(height):
	...
	return [grid[x][y] + random.randint(-1, 0) if x != center[0] and y != center[1] else grid[x][y] for x in range(len(grid)) for y in range(len(grid[x]))]
	...
	for e in range(start_episode + 1, episodes + 1):
		...
		<Mission xmlns="http://ProjectMalmo.microsoft.com" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
		  ...
		  <ServerSection>
		    ...
		    <ServerHandlers>
		      <FlatWorldGenerator generatorString="3;1*minecraft:cobblestone;2" forceReset="true"/>
		  ...
		  <AgentSection mode="Survival">
		    <Name>Bob</Name>
		    <AgentStart>
		      <Placement x="{}.5" y="{}" z="{}.5" pitch="-90" yaw="0"/>
		    </AgentStart>
		    <AgentHandlers>
		...
		while world_state.is_mission_running:
			...
			if world_state.number_of_observations_since_last_state > 0:
				...
				reward = 0
				if prev_x != current_x or prev_y != current_y or prev_z != current_z:
					if abs(prev_y - current_y) <= 25:
						reward = prev_y - current_y
					else:
						if prev_y > current_y:
							reward = 25
						else:
							reward = -25
					reward -= 50 * damage_taken
				else:
					reward = -1000
				done = True if get_current_block(data) == 'cobblestone' or not world_state.is_mission_running or data['Life'] <= 0 else False
