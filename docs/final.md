...
# Summary
For our project, we wanted our agent to find the most optimal path down a given hill. Some of the hills we trained our agent on, were actual hills found in randomly generated Minecraft worlds while others were generated by us to test our agent's ability. In Minecraft, our agent would take damage if he attempted to fall more than 3 blocks. Every time our agent takes damage, it gets penalized (we give our agent a negative reward). If it manages to take a step down the hill without taking damage, we give it a positive reward. Our agent keeps making moves until it has either taken 100 steps or the agent reached the bottom of the hill.

Our agent has the ability to see the heights of the blocks which it can move to in two moves. As you can see in the grid below, we assume our agent is located in the yellow square. The agent has the ability to see all of the relative heights of the green squares. All the red squares are locations which the agent cannot reach in two moves so we remove them from our model.
